...
// NOTE Add preloaded modules to cache
    for (var id in (self.Honey && self.Honey.requirePreloaded))
        cache['$'+resolvejs(id).id] = self.Honey.requirePreloaded[id].toString();

// NOTE Add module overrides to cache
    for (var id in (self.Honey && self.Honey.requireOverrides))
        cache['$'+resolvejs(id).id] = self.Honey.requireOverrides[id];
...
//      module has been loaded.
    ...
    function requirejs(identifier, callback, compiler) {
        if (identifier instanceof Array) {
            var modules = new Array();
            var modcount = identifier.length;
            for (var index = 0; index < identifier.length; index++) {
                (function(id, i) {
                    modules.push(requirejs(id, callback&&function(mod) {
                        modules[i] = mod;
        ...
        var descriptor = resolvejs(identifier);
...
// INFO Exporting require to global scope
    ...
    if (self.requirejs !== undefined)
        throw new Error("Honey: '\'require\' already defined in global scope");

    try {
        Object.defineProperty(self, 'requirejs', {'value':requirejs});
        Object.defineProperty(self.requirejs, 'resolve', {'value':resolvejs});
        Object.defineProperty(self.requirejs, 'path', {'get':function(){return requirePath.slice(0);}});
    }
    catch (e) {
        ...
        self.requirejs.resolve = resolvejs;
        self.requirejs.path = requirePath.slice(0);
    }
