var kCompleter = {
    ...
    getCompletions: function (editor, session, pos, prefix, callback) {
        ...
        if (session.$struct && session.$struct.length > 0) {
            ...
            for(let s of scope){
                let topLevel = s.children.filter(namedScopeFilter);
                for (let elm of topLevel) {
                    keywords[elm.name] = elm.type;
                }
                if (prevTok && prevTok.type === "punctuation.member") {
                    let members = tokI - 2 >= 0 ? findType(s, toks[tokI - 2].value) : [];
...
function findScope(node, tree){
    ...
    //     return min;
    // };
    const lt = (p1,p2) => (p1.line < p2.line) || (p1.line === p2.line) && (p1.col < p2.col);

    let scopes = dfs(tree[0],'child.children && child.children.length > 0',[]);
    let accessibleScopes = scopes
                            .filter(scope => lt(scope.start, node.start))
                            .filter(scope => lt(node.end, scope.end));
